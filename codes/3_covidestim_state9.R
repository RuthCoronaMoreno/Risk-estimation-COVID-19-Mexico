...
# Script     : 3_covidestim_state9.R
# Purpose    : covidestim estimation for state 9 (state level).
#
#Requirements: Run it in the supercomputer of "National Laboratory for Advanced Scientific Visualization (LAVIS-UNAM)"
#              by using the code "run_s9_full.slurm" 
#
...
#
# Input      : incidence_states_mx.csv
#              deaths_states_mx.csv
#              population_mx.csv
...
#databases
I=read_csv("data/processed/incidence_states_mx.csv")
D=read_csv("data/processed/deaths_states_mx.csv")
popcsv=read_csv("data/raw/population_mx.csv")


#Name and total population of state (2020)
if(state<10){
  name=paste0("0",state)
}else{
...
pop=sum(popcsv$POBTOT[popcsv$CLAVE_REGION==state])
...
#Save output
dir_path <- "data/covidestim/states"
if (!dir.exists(dir_path)) {
  dir.create(dir_path, recursive = TRUE)
} 
write.csv(dI, paste0(dir_path,"/",name,"_covidestim.csv"))
